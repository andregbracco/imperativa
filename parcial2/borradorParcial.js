...
//  id 8 ok, Neumonología, Guerra Bright, valor consulta $1949.92, puntaje: 0
  listarProfesionales: function(lista) {
    for (let index = 0; index < lista.length; index++) {
      let disponibilidad = lista[index].estaHabilitado ? 'ok' : 'inactivo'
      console.log(
        `id ${lista[index].id} ${disponibilidad}, ${lista[index].especialidad}, ${lista[index].nombre}, valor consulta $ ${lista[index].honorarioConsulta}, puntaje: ${lista[index].puntaje} `
      )     
    }
  },
  filtrarMejoresPuntajes: function() {
    ...
    return this.listarProfesionales(mejoresPuntajes)
  }
}
